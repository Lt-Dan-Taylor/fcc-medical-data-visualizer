...
def draw_cat_plot():
  ...
  df_cat = df_cat.groupby(['variable', 'value',
                           'cardio']).size().reset_index(name='total')
...
def draw_heat_map():
  ...
  # Clean the data
  df_heat = df.loc[(df['ap_lo'] <= df['ap_hi']) & (df['height'].between(
  ...
  sns.heatmap(ax=ax,
              ...
              vmax=0.32,
              cbar_kws={
                  'boundaries': np.arange(-0.16, 0.32, 0.01),
